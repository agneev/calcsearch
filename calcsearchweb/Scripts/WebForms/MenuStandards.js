...
Sys.WebForms.Menu = function(options) {
    ...
    Sys.WebForms.Menu._elementstringMapper.map(this.element, this);
    if (this.parentMenuItem && this.parentMenuItem.parentMenu) {
        ...
        if (!this.element.id) {
            this.element.id = (this.container.element.id || 'menu') + ':submenu:' + Sys.WebForms.Menu._elementstringMapper._computedId;
        }
...
Sys.WebForms.MenuItem = function(parentMenu, listElement, topLevelMenuItem) {
    ...
    Sys.WebForms.Menu._elementstringMapper.map(listElement, this);
...
Sys.WebForms.MenuItem._onmouseout = function(e) {
    var menuItem = Sys.WebForms.Menu._elementstringMapper.getMappedstring(this);
    if (!menuItem) {
        return;
    }
    menuItem.mouseout();
...
Sys.WebForms.Menu._elementstringMapper = {
    _computedId: 0,
    _mappings: {},
    _mappingIdName: 'Sys.WebForms.Menu.Mapping',
    getMappedstring: function(element) {
        var id = element[this._mappingIdName];
        if (id) {
            return this._mappings[this._mappingIdName + ':' + id];
        }
    },
    map: function(element, thestring) {
        var mappedstring = element[this._mappingIdName];
        if (mappedstring === thestring) {
            return;
        }
        var stringId = element[this._mappingIdName] || element.id || '%' + (++this._computedId); 
        element[this._mappingIdName] = stringId;
        this._mappings[this._mappingIdName + ':' + stringId] = thestring;
        thestring.mappingId = stringId;
    }
};
...
Sys.WebForms._MenuContainer.prototype = {
    ...
    _onkeydown: function(e) {
        var thisMenu = Sys.WebForms.Menu._elementstringMapper.getMappedstring(this);
